...
const resolvers = {
    Query: {
        ...
        news: async (parent, { nextPage }) => {
            ...
            if (cached)
                return cached;
            // Fetch if no cache
            let url = "https://newsdata.io/api/1/news?apikey=pub_394997602ee65c4a146b37046655c30c98f8a&language=en&q=crypto";
            if (nextPage)
                url += `&page=${nextPage}`;
    ...
    Subscription: {
        ticker: {
            subscribe: withFilter((parent, args) => {
                ...
                return variables.symbols.includes(payload.ticker.symbol);
            }),
        },
    },
};
